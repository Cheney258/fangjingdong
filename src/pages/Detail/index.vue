        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转从A路由跳转到B路由，这里再加入购物车进行路由跳转之前
                把购买的产品的信息通过请求的形式通知服务器，服务器再进行相应的存储-->
...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      // 给子组件的数据
      skuImageList(){
        // 如果服务器的数据没有回来，skuInfo.skuImageList至少是个空数组
    ...
    methods:{
      ...
      // 加入购物车的回调函数
      async addShopCar(){
        // 派发actions，1.发请求，将产品添加到服务器
      try {
        await this.$store.dispatch('addOrUpdateShopCart',
        {skuId:this.$route.params.skuid,skuNum:this.skuNum});
        // 一些简单的数据skuNum，通过query形式给路由组件传递下去
        // 产品的信息比较复杂【skuInfo】，通过会话存储（不持久化，会话结束数据就就会消失）
        // 本地存储|| 会话存储 ，一般存储的是字符串
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
        // 路由跳转
        // 在路由跳转时，将产品信息传递给下一级组件
